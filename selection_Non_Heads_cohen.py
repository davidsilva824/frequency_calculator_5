...
# --- (MODIFIED) Only the 'primary' variable logic is changed ---
def score_combo(packs_subset):
    K = len(packs_subset)
    
    # (MODIFIED) Must store all values for Cohen's d
    # groups x columns x [list of values]
    group_values = [[[] for _ in range(3)] for _ in range(4)] 
    
    for P in packs_subset:
        for gi, key in enumerate(GROUP_KEYS):
            _, s, b10, b100 = P[key]
            group_values[gi][0].append(s)
            group_values[gi][1].append(b10)
            group_values[gi][2].append(b100)
            
    # (MODIFIED) Calculate means from the new structure
    means = [[sum(group_values[g][c])/K for c in range(3)] for g in range(4)]
    ...
    # --- Calculate requested Cohen's d *using your module* ---
    ...
    # primary: minimize the worst (max absolute) Cohen's d
    all_cohen_d_values = [abs(d) for d in cohen_d_sg + cohen_d_pl]
    primary = max(all_cohen_d_values)
    # -----------------------------------
