This testing pipeline is part of the masters dissertation: "Exploring Grammatical Constraints with Large Language Models" by David Silva, supervised by João Veríssimo and Vânia Mendonça.

It calculates Zipf values for subtlex-UK, two datasets from the babyLM challenge 2024, and the British National Corpus (BNC). 
https://osf.io/zq49t/
https://osf.io/ryjfm/overview
https://www.english-corpora.org/bnc/

Install all requested packages. Run the 'zipf_list_of_words.py' once in order to download the datasets. 

Run 'zipf_list_of_words.py' file to obtain the Zipf values for one or more words.

With 'selection_Non_Heads_mean_or_cohen.py' file to obtain the best lists of Non-Head combinations, from the 24 considered packs of 1 irregular pair and 1 semanically similar regular pair. You can select different criteria.

Run 'frequency_stimuli' files to check for the absolute frequency of compounds in in the BabyLM datasets (10M and 100M).
To obtain the frequency of examples in the B

zipf_stats and zipf_calculator modules are classes with methods. 
